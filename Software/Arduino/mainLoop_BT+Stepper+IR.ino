...
const int trigPin_scoop = 34;
const int echoPin_scoop = 35;

const int sensors[7][2] = {
    {trigPin_1, echoPin_1},
    {trigPin_2, echoPin_2},
    {trigPin_3, echoPin_3},
    {trigPin_4, echoPin_4},
    {trigPin_5, echoPin_5},
    {trigPin_6, echoPin_6},
    {trigPin_scoop, echoPin_scoop},
};

const int ir_sensors[2] = {A8, A9};

const int LOOP_DELAY = 50; // ms
float last_angle = 10; // deg
...
{   
    ...
    pinMode(trigPin_scoop, OUTPUT); 
    pinMode(echoPin_scoop, INPUT); 
    ...
    {   
        ...
        for(int i=0; i<7; i++)
        {
            for (int j=0; j<3; j++) 
            {
                ...
                char char_dist[10];
                dtostrf(distance, 7, 2, char_dist);
        ...
        for(int j=0; j<2; j++){
            for(int i=0; i<3; i++){
                int val = analogRead(ir_sensors[j]) * 0.0048828125;
...
char distToChar(float* distances)
{
    ...
    {
        char char_dist[10];
        dtostrf(distances[i], 7, 2, char_dist);
